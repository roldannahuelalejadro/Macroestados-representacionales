...
\title{
Macroestados representacionales y dificultad estadística en prompts de seguridad\\
...
Presentamos un formalismo procedural para estudiar prompts de seguridad en LLMs como un problema inverso. En el problema directo, un prompt $q$ y una condición experimental $C$ inducen una respuesta observable o, más generalmente, una población de trayectorias generativas. En el problema inverso de seguridad, el objetivo es encontrar, monitorear o controlar prompts, condiciones e intervenciones para que esa población permanezca cerca de una especificación segura $S^\star$.

Dentro de este formalismo definimos nuestro benchmark como una dificultad estadística poblacional $D_{\mathrm{stat}}$, estimada a partir de rollouts. Cada trayectoria se puntúa mediante un vector de errores que distingue fuga exacta, fuga parcial, transformación reversible, cooperación insegura, falso rechazo y fallo de tarea. En las corridas conductuales con $qwen3\text{-}coder{:}30b\text{-}32k$, observamos que la dificultad se concentra principalmente en transformaciones de extracción y restricciones de formato, mientras que la inyección de contexto aislada produce una señal más débil.

Luego, para modelizar los resultados del benchmark, proponemos una descripción representacional efectiva. Introducimos coordenadas proxy $\xi(\alpha)$ y ajustamos una jerarquía de modelos $D(\xi)$, incluyendo un Hamiltoniano cuadrático efectivo con matriz de acoplamientos $H$. Los resultados sugieren que los términos cuadráticos no diagonales aportan poder predictivo sobre modelos constantes, lineales y diagonales. Finalmente, estudiamos composiciones de prompts mediante diferencias finitas $I_{AB}$ y scattering representacional $S_{AB}$, encontrando que la no aditividad conductual mezcla acoplamientos efectivos con curvatura del mapa prompt-representación.
...
\begin{enumerate}[label=(\roman*),leftmargin=*,itemsep=0pt,topsep=1pt,parsep=0pt]
    \item Formulamos el estudio de prompts de seguridad en LLMs como un problema inverso.
    \item Definimos un benchmark conductual poblacional mediante $D_{\mathrm{stat}}$.
    \item Introducimos una descripción representacional proxy en coordenadas reducidas $\xi$ y un modelo efectivo $D(\xi)$ con acoplamientos $H$.
    \item Analizamos composiciones de prompts mediante interacciones finitas $I_{AB}$ y scattering representacional $S_{AB}$.
\end{enumerate}
La Figura~\ref{fig:pipeline} resume la arquitectura conceptual del informe.
}
...
% Blocks
\node[block, fill=violet!14] (prompt) {
    \textbf{Prompt scenario}\\[-1pt]
    \(q_\alpha\)
};

\node[block, fill=cyan!14, below=of prompt] (rollouts) {
    \textbf{Rollouts}\\[-1pt]
    \(\mathcal{P}_\alpha=\{\gamma_{\alpha,b}\}_{b=1}^{B}\)
};

\node[block, fill=green!14, below=of rollouts] (scoring) {
    \textbf{Scoring}\\[-1pt]
    \(e_{\alpha,b}\)
};

\node[block, fill=orange!18, below=of scoring] (dstat) {
    \textbf{Behavioral benchmark}\\[-1pt]
    \(D_{\mathrm{stat}}(\alpha)\)
};

\node[block, fill=red!10, below=of dstat] (proxy) {
    \textbf{Proxy representation}\\[-1pt]
    \(\xi(\alpha)\)
};

\node[block, fill=magenta!12, below=of proxy] (effective) {
    \textbf{Effective model}\\[-1pt]
    \(D(\xi),\,H\)
};

\node[block, fill=pink!8, below=of effective] (comp) {
    \textbf{Compositions}\\[-1pt]
    \(I_{AB},\,S_{AB}\)
};
...
% Side arrow: Effective model approximates the benchmark
...
El punto de partida es una regla directa muy simple. La lectura como problema inverso toma inspiracion de formulaciones de diseño inverso, donde se busca una entrada o configuracion capaz de producir una respuesta objetivo, a menudo en presencia de no unicidad~\cite{liu2018training,molesky2018inverse}. Dado un prompt \(q\) y una condicion experimental \(C\), el modelo produce un resultado observable:
\[
...
Cambiar alguno de estos elementos cambia el problema experimental. En particular, cambiar el modelo, el tokenizer, el prompt de sistema o el decoding cambia la distribucion de trayectorias aun si el texto visible del usuario es el mismo. En modelos Transformer, los estados ocultos por capa y posicion constituyen la base operacional sobre la que luego construiremos observables reducidos~\cite{vaswani2017attention}.
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=1\linewidth]{fig_macrostate_patterns.png}
    \caption{
...
La Figura~\ref{fig:macrostate_activation_patterns} da una intuición esquemática
de esta idea: cada transformación induce un patrón efectivo de activación,
pero la composición \(AB\) no tiene por qué coincidir con una superposición
aditiva de los patrones inducidos por \(A\) y \(B\).
...
La Figura~\ref{fig:strongest_transform_cells} muestra las celdas familia--intensidad con mayor dificultad media en el barrido amplio. Este resumen visual motiva concentrar la medición focal en las familias más directamente ligadas a extracción y composición: \(\Ctrans\), \(\Ptrans\) y \(\Ttrans\).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{fig_dstat_strongest_cells.png}
...
El barrido amplio mostro que varias familias producian señal. Las restricciones de formato fueron fuertes, y tambien aparecieron cambios de idioma, parafrasis y justificaciones inocentes. Sin embargo, para la medicion principal elegimos tres ejes directamente ligados a la pregunta de seguridad y a la composición posterior de ataques:
\[
\Ctrans,\qquad \Ptrans,\qquad \Ttrans.
\]

Estos cubren, respectivamente, conflicto contextual de instrucciones, extraccion parcial y transformacion reversible del secreto. La medicion focal no intentaba repetir todo el barrido; aumentaba el numero de rollouts por prompt en las familias mas utiles para estudiar acoplamientos.
...
La matriz por componentes del reporte HTML es demasiado grande para incluirla completa en el cuerpo principal; el reporte completo queda referenciado en el Apéndice~\ref{app:repo}. Su lectura resumida es:
\begin{center}
\scriptsize
\begin{tabular}{@{}lcl@{}}
\toprule
...
V=\{v_1,\dots,v_K\}.
\]
Estas direcciones pueden provenir de RepE/LAT, diferencias de activaciones entre condiciones seguras e inseguras, PCA de deltas, o una base proxy ajustada empiricamente. La motivacion general esta alineada con representation engineering, que propone estudiar fenomenos de alto nivel mediante representaciones poblacionales en espacios de activacion~\cite{zou2023representation}.
...
\]
Los modelos se ajustaron por cuadrados minimos regularizados sobre features polinomiales de \(\xrep\), con regularizacion seleccionada por validacion interna. La evaluacion principal fue leave-secret-out. Las tablas procesadas y scripts de ajuste correspondientes se listan como material suplementario en el Apéndice~\ref{app:repo}.
...
El termino scattering se usa aqui como analogia geometrica: no pretendemos resolver un problema de dispersion fisica, sino medir cuanto se aparta una composicion textual de la suma lineal de desplazamientos representacionales. La inspiracion formal viene de la relacion entre sistemas no lineales, scattering inverso y estructuras Hamiltonianas~\cite{zakharov1972exact,faddeev2007hamiltonian}.
...
Esta no aditividad no debe identificarse automaticamente con una entrada de \(H\). Puede haber un acoplamiento real en \(D^{(2)}\), es decir un termino cruzado de la forma \(\xi_i\xi_j\), pero tambien puede ocurrir que la composicion del prompt cambie las coordenadas representacionales que estamos evaluando. La Figura~\ref{fig:geom_scattering} resume esta distincion: \(\delta\xi_A^\top H\delta\xi_B\) mide un acoplamiento efectivo en una geometria local fija, mientras que \(S_{AB}\) mide el fallo de composicion lineal en el mapa prompt--representacion.
...
+\xrep_0.
\]
La Figura~\ref{fig:geom_scattering} visualiza esta diferencia entre la prediccion lineal \(\delta\xi_A+\delta\xi_B\) y la representacion observada de la composicion \(\delta\xi_{AB}\). El punto clave es que, si \(S_{AB}\neq0\), la no aditividad conductual \(I_{AB}\) mezcla el acoplamiento efectivo local con el desplazamiento nuevo inducido por la composicion.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.82\textwidth]{fig_geom_scattering.png}
...
Como chequeo adicional, se repitio el par \(\Ptrans+\Ttrans\) en el modelo grande con prompts nuevos, \(5\) secretos, \(8\) bases y \(B=32\), totalizando \(5120\) rollouts. Los prompts y tablas de esta corrida se conservan como material suplementario en el repositorio asociado. El acoplamiento observado fue antiaditivo:
\[
...
\section{Material suplementario y repositorio asociado}
\label{app:repo}

El informe principal conserva solamente las definiciones, resultados agregados y figuras necesarias para seguir el argumento. Los archivos completos de corrida, tablas procesadas, reportes auxiliares y scripts de reproduccion se dejan en un repositorio asociado:

\begin{center}
\href{https://github.com/roldannahuelalejadro/Macroestados-representacionales}{\texttt{Repositorio: Macroestados-representacionales}}
\end{center}

En particular, el repositorio funciona como material suplementario para los elementos que no conviene incluir completos en el PDF:

\begin{itemize}
    \item prompts base, prompt de sistema con la boveda sintetica y familias de transformaciones;
    \item reportes HTML y salidas auxiliares de las corridas conductuales;
    \item tablas procesadas con vectores de error \(e_{\alpha,b}\), agregadores y valores de \(\Dstat\);
    \item CSVs usados para los ajustes de regresion, clasificacion, jerarquia \(D^{(n)}\) y diagnosticos de \(H\);
    \item archivos y tablas derivados de activaciones proxy, PCA y coordenadas \(\xrep\);
    \item scripts o notebooks usados para reproducir las figuras y resultados resumidos en el informe;
    \item tablas de composiciones, diferencias finitas \(I_{AB}\), normas de scattering \(S_{AB}\) y chequeos de no conmutatividad.
\end{itemize}

Esta separacion busca mantener el cuerpo principal legible, pero dejando una ruta clara para auditar de donde salen las tablas, figuras y numeros reportados.
